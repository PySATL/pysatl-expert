...
from scripts.training_data import load_frozen_training_dataset, load_frozen_training_partition
...
def test_load_frozen_training_dataset_returns_the_two_partitions(tmp_path: Path):
    ...
    for target, split, value in (
        ...
        row: dict[str, Any] = {feature: value for feature in FeatureVector.FEATURE_NAMES}
...
def test_load_frozen_training_dataset_rejects_unknown_split(tmp_path: Path):
    row: dict[str, Any] = {feature: 1.0 for feature in FeatureVector.FEATURE_NAMES}
